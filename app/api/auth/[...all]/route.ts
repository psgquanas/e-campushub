...
async function protect(
  ...
  // If this is a signup or email verification request then use the special protectSignup rule
  // See https://docs.arcjet.com/signup-protection/quick-start
  if (
    req.nextUrl.pathname.startsWith("/api/auth/sign-up") ||
    req.nextUrl.pathname.startsWith("/api/auth/email-otp/send-verification-otp")
  ) {
...
export const POST = async (req: NextRequest) => {
  ...
  const decision = await protect(requestToUse, parsedBody);
  ...
  return authHandlers.POST(requestToUse);
};
